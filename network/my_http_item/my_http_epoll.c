...
static void* DealRequest(void* arg)
{
    ...
    int client_sock = *((int*)arg + sizeof(int));
...
//处理就绪的文件描述符集
void Server(int epoll_fd, int listen_sock, struct epoll_event events[], int num)
{
    int i = 0;
    for(; i < num; ++i)
    {
        ...
        }else if(events[i].events & EPOLLIN && events[i].events & EPOLLOUT)
        {
            ...
            //创建线程，执行Server函数
            pthread_t tid;
            pthread_create(&tid,NULL,DealRequest,arg);
            pthread_detach(tid);
        }else
        {
            return;
...
int main(int argc, char* argv[])
{
    ...
    //创建epoll句柄
    int epoll_fd = epoll_create(MAXEVENT+1);
    if(epoll_fd == -1)
    {
        perror("epoll_create");
        return 2;
    ...
    for(;;)
    {
        ...
        int ret = epoll_wait(epoll_fd,events,MAXEVENT,-1);
        switch(ret)
        {
            case 0:
            ...
            default:
                printf("%d\n",__LINE__);
                Server(epoll_fd,listen_sock,events,ret);
                break;
        }
    }
